...
class FacadeFolder(Dataset):
    def __init__(self, path_A, path_B, imsize=None, cropsize=None, cencrop=False):
...
def imload(path, imsize, cropsize, cencrop):
    transformer = []
    if imsize:
        transformer.append(transforms.Resize(imsize))
    if cropsize:
        if cencrop:
            transformer.append(transforms.Cencrop(cropsize))
        else:
            transformer.append(transforms.RandomCrop(cropsize))
    transformer.append(transforms.ToTensor())
    transformer.append(_normalize())
